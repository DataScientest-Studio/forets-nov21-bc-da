...
csv_files = [
    "burnedData.csv",
    "climatology1991_2015.csv",
    "data.csv",
    "perfectData.csv",
    "sunData.csv",
    "surface_brulee_etat_annee.csv"
]

base_url = "https://projet-nov21bda-feux-de-forets.s3.eu-west-1.amazonaws.com/"


for csv_file in csv_files:
    if csv_file not in files:
        print(f"Downloading '{csv_file}'")
        wget.download(base_url + csv_file)
...
# ========= Nombre de feux=============================================================
    ...
    for i in top5:
        ...
        d[i] = d[i].reset_index()
        d[i] = d[i].sort_values('index').merge(
            pd.DataFrame(temp[i]).reset_index())
    ...
    df_tree = sun2.groupby("state").agg(
        {"size_km2": "sum", "fire_number": "sum"})
    ...
    st.subheader(
        "Emplacement idéal des super-casernes pour combattre les deux de classe G")
